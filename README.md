...
- ✅ **每日限制机制**：各项积分获取行为均有每日上限控制
- ✅ **乐观锁并发控制**：使用 version 字段 + 3 次重试机制解决并发积分更新冲突
- ✅ **等级保护期**：积分减少后 7 天内不降级
- ✅ **自动升级/降级**：每次积分变动后自动重新计算等级
...
- ✅ **等级图标资源**：26 张等级图标图片（13 个等级 × 标准/2x 双分辨率）
...
| `bbs_point_record`   | 积分变动记录表（按月分区） |
| `bbs_level_config`   | 等级配置表（含 JSON 特权定义） |
...
如需手动执行，按编号顺序运行 `java-server/src/main/resources/sql/` 目录下的 SQL 文件（从 `00_full_reset.sql` 到 `31_fix_report_deleted_posts.sql`，跳过 `99_` 开头的调试脚本）。
...
- 新增历史数据修复脚本 `31_fix_report_deleted_posts.sql`，用于修正旧举报处理记录对应的帖子状态
