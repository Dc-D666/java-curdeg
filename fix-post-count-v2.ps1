# fix-post-count-v2.ps1 - 发帖数数据一致性修复工具 V2

Write-Host "==========================================" -ForegroundColor Cyan
Write-Host "   发帖数数据一致性修复工具 V2" -ForegroundColor Cyan
...
try {
    $fixResponse = Invoke-WebRequest -Uri "$baseUrl/api/bbs/user/fix-post-count" `
        -Method POST `
    ...
    if ($fixContent.code -eq 200 -or $fixContent.code -eq 0) {
        ...
        if ($fixContent.data.fixedUsers -eq 0) {
            ...
            Write-Host "已成功修复 $($fixContent.data.fixedUsers) 个用户的数据！" -ForegroundColor Green
        }
    } else {
